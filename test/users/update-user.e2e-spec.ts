...
describe('PATCH /v1/users/:id', () => {
  ...
  it('updates a user and returns the success envelope', async () => {
    const res = await request(ctx.app.getHttpServer())
      .patch('/v1/users/42')
      .send({})
      .expect(200);
    const body = asSuccess<{ id: number; updated: boolean }>(res.body);

    expect(body.data).toEqual({ id: 42, updated: true });
